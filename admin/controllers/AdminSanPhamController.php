...
class AdminSanPhamController
{
    ...
    public function postAddSanPham()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($hinh_anh['error'] != 0) {
                $errors['hinh_anh'] = 'Vui lòng phải chọn ảnh sản phẩm ';
            }
            if (empty($dung_tich)) {
                $errors['dung_tich'] = 'Vui lòng nhập dung tich';
            }
            $_SESSION['error'] = $errors;
            // var_dump($errors); die();
            if (empty($errors)) {
               $san_pham_id = $this->modelSanPham->insertSanPham($ten_san_pham, $gia_san_pham, $gia_khuyen_mai, $so_luong, $ngay_nhap, $danh_muc_id, $trang_thai, $mo_ta ,$file_thumb, $dung_tich);
